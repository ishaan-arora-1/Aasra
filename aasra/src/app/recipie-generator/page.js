...
const Page = () => {
  ...
  const handleGenerateRecipe = () => {
    // Create text file with ingredients
    const blob = new Blob([ingredients], { type: 'text/plain' });
    ...
    reader.onload = function(e) {
        const tempContent = e.target.result;
        const tempFile = new Blob([tempContent], { type: 'text/plain' });
        const tempUrl = URL.createObjectURL(tempFile);
        
        // Download temp file
        const link = document.createElement('a');
        link.href = tempUrl;
        link.download = 'temp-ingredients.txt';
        ...
        setTimeout(() => {
            ...
            // Delete file (this will only remove from downloads, not filesystem)
            link.remove();
        }, 30000);
    };
    reader.readAsText(blob);

    // Your existing recipe generation code here
    // ...

    // Only revoke the URL when you're done with it
